...
1. Build a $4 \times 3$ matrix with all 1s. Multiply by a $3 \times 4$ matrix with all 2s.
...
4. Make $\mathbf{A}=\left[ \begin{smallmatrix}1&2\\3&4\end{smallmatrix}\right]$.  Build a $\mathbf{B}$ diagonal matrix such that $\mathbf{A}\mathbf{B}=\left[\begin{smallmatrix}1&6\\2&8\end{smallmatrix}\right]$. Show your R code for $\mathbf{A}$, $\mathbf{B}$ and $\mathbf{A} \mathbf{B}$.
...
1. Build $\mathbf{A}$, a $4 \times 3$ matrix. Show how to write $\mathbf{A}\mathbf{A}^\top$ in R. The $\top$ means transpose.
